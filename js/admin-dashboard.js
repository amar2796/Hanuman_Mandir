    ...
    function _hmInitSelectors() {
      ...
      if (ydrop) {
        ydrop.innerHTML = sortedYears.map(function(y) {
          return '<div onclick="_hmSelectYear(' + y + ')" style="padding:8px 16px;font-size:13px;font-weight:600;color:#e2e8f0;cursor:pointer;transition:background 0.15s;white-space:nowrap;" onmouseover="this.style.background=\'rgba(193, 68, 14,0.15)\'" onmouseout="this.style.background=\'\'">' + y + '</div>';
        }).join("");
      }
    ...
    function _hmToggleYearDropdown(forceClose) {
      ...
      if (open) {
        ...
        Array.from(drop.children).forEach(function(el) {
          el.style.color = el.textContent.trim() === String(_hmSelYear) ? "#C1440E" : "#e2e8f0";
    ...
    function _hmRefreshBar() {
      ...
      if (lbl) {
        lbl.style.background = isCurrent
          ? "linear-gradient(90deg,#C1440E,#f59e0b)"
          : "linear-gradient(90deg,#C1440E,#C1440E)";
      }
    ...
    function _hmRenderMonthPills(activeMonth) {
      ...
      container.innerHTML = _hmShortMonths.map(function(m, i) {
        ...
        return '<div onclick="_hmSelectMonth(\'' + fullName + '\')" style="' +
          ...
          'background:' + (isActive ? 'linear-gradient(135deg,#C1440E,#f59e0b)' : 'rgba(255,255,255,0.06)') + ';' +
          'color:' + (isActive ? '#fff' : '#94a3b8') + ';' +
          'border:1.5px solid ' + (isActive ? 'rgba(193, 68, 14,0.6)' : 'rgba(255,255,255,0.09)') + ';' +
          'cursor:pointer;user-select:none;transition:all 0.18s;white-space:nowrap;' +
          'box-shadow:' + (isActive ? '0 2px 12px rgba(193, 68, 14,0.35)' : 'none') + ';' +
    ...
    function _hmRenderOccasionBreakdown(monthContribs) {
      ...
      el.innerHTML = rows.map(function(r) {
        var bg  = r.neutral ? "#f1f5f9" : "#FDF0E6";
        var bd  = r.neutral ? "#e2e8f0" : "#F3C6A8";
        var col = r.neutral ? "#64748b" : "#C1440E";
    ...
    function _hmRenderWalkinList(walkIns) {
      ...
      el.innerHTML = recent.map(function(c) {
        ...
        return '<div style="display:flex;align-items:center;gap:8px;padding:7px 10px;border-radius:8px;background:#fffbeb;margin-bottom:5px;">' +
          '<div style="width:28px;height:28px;border-radius:50%;background:#E0987A;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#92400e;flex-shrink:0;">W</div>' +
          '<div style="flex:1;min-width:0;">' +
    ...
    function _hmRenderBarChart(curYear, selMonth) {
      ...
      var rows = active.map(function(m) {
        ...
        return '<div onclick="_hmSelectMonth(\'' + m + '\')" ' +
          ...
          (sel ? 'background:rgba(193, 68, 14,0.07);border-radius:7px;padding:4px;' : 'padding:3px 4px;') + '">' +
      ...
      var chips = prevM
        ? '<div style="display:flex;gap:6px;margin-top:10px;padding-top:8px;border-top:0.5px solid #f1f5f9;">' +
            ...
            '<div style="flex:1;background:#FDF0E6;border-radius:8px;padding:7px 10px;text-align:center;min-width:0;">' +
              '<div style="font-size:10px;color:#64748b;margin-bottom:2px;">Net · ' + selMonth.slice(0,3) + '</div>' +
              '<div style="font-size:12px;font-weight:600;color:' + (netSel>=0?'#C1440E':'#dc2626') + ';white-space:nowrap;">' +
    ...
    function _hmRenderAlerts(pendingCount, monthE, activeMembers, curMonth, curYear) {
      ...
      el.innerHTML = alerts.map(function(a) {
        var bg  = a.type === "warn" ? "#fffbeb" : a.type === "info" ? "#FDF0E6" : a.type === "birthday" ? "#fdf4ff" : "#f0fdf4";
        var bc  = a.type === "warn" ? "#E0987A" : a.type === "info" ? "#bfdbfe" : a.type === "birthday" ? "#e9d5ff" : "#bbf7d0";
        var ic  = a.type === "warn" ? "fa-triangle-exclamation" : a.type === "info" ? "fa-circle-info" : a.type === "birthday" ? "fa-cake-candles" : "fa-circle-check";
        var ic2 = a.type === "warn" ? "#d97706" : a.type === "info" ? "#C1440E" : a.type === "birthday" ? "#9333ea" : "#16a34a";
    ...
    function _hmRenderYearTracker(curYear, curMonth) {
      ...
      if (el) {
        ...
        el.style.background = pct >= 75 ? "#27ae60" : pct >= 40 ? "#C1440E" : "#e74c3c";
      }
      ...
      (function _renderYearStatCells() {
        ...
        wrap.innerHTML =
          ...
          cell("Members",     String(memberCount), "#C1440E");
      })();
    }
