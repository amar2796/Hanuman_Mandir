var _ct_filtered   = [];   // filtered contribution rows (with pending injected)
        ...
        tbody.innerHTML = slice.map(row => {
          if (row._type === "paid") {
            ...
            return `<tr class="clickable-row" style="cursor:pointer;" onclick="viewDashboardEntry('${_drid}')">
              ...
              <td><span style="background:#FDF0E6;color:#7A1F1F;padding:2px 6px;border-radius:4px;font-size:9px;font-weight:700;font-family:monospace;">${escapeHtml(rid||"—")}</span></td>
    ...
    function dash_renderCal() {
      ...
      for (let d = 1; d <= daysInMonth; d++) {
        ...
        if (inRange) { bg = "#FDF0E6"; co = "#C1440E"; }
        if (isToday && !isSel) { extra = "box-shadow:inset 0 0 0 1.5px #C1440E;"; co = "#C1440E"; fw = 700; }
        if (isSel) { cls += " dc-sel"; bg = "#C1440E"; co = "#fff"; fw = 700; }
    ...
    function fld_renderCal() {
      ...
      for (let d = 1; d <= daysInMonth; d++) {
        ...
        if (isToday && !isSel) { extra = "box-shadow:inset 0 0 0 1.5px #C1440E;"; co = "#C1440E"; fw = 700; }
        if (isSel) { bg = "#C1440E"; co = "#fff"; fw = 700; }
...
(function () {
    ...
    */
    ...
    function _build() {
      ...
      inp.addEventListener('focus', function () { this.style.borderColor = '#C1440E'; });
    ...
    function _lookup() {
      ...
      res.innerHTML =
        '<div style="border:1px solid #E0987A;border-radius:10px;overflow:hidden;">'
        + '<div style="background:linear-gradient(90deg,#C1440E,#f5b942);padding:9px 13px;'
        ...
        + '</div>'
        + '<div style="padding:10px 13px;background:#fffdf5;border-top:1px solid #E0987A;">'
          + '<button id="grsViewBtn" style="width:100%;padding:11px 0;'
          + 'background:linear-gradient(135deg,#C1440E,#e8920a);color:#fff;border:none;'
          + 'border-radius:8px;cursor:pointer;font-size:13px;font-weight:700;'
          + 'font-family:Poppins,sans-serif;box-shadow:0 3px 10px rgba(193, 68, 14,0.3);'
