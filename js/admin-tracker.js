  ...
  allMemersList.forEach(member => {
    ...
    html += `
      <div style="display:flex;align-items:center;gap:10px;padding:10px;border-bottom:1px solid #e2e8f0;">
        <input type="checkbox" id="${checkboxId}" data-userid="${member.UserId}" data-ispaid="${isPaid ? '1' : '0'}" style="width:16px;height:16px;cursor:pointer;accent-color:#C1440E;flex-shrink:0;">
...
function _trRenderLineChart(items, opts) {
  ...
  const dots = points.map((p, i) => `
    <circle class="tr-dot" style="animation-delay:${600 + i * 60}ms" cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="3.5" fill="#C1440E"><title>${_trEsc(p.label)}: ${p.value}%</title></circle>
  ...
  return `
    ...
    <svg viewBox="0 0 ${w} ${h}" style="width:100%;height:${h}px;display:block;">${grid}<path class="tr-line-draw" d="${pathD}" fill="none" stroke="#C1440E" stroke-width="2"/>${dots}</svg>`;
}
    ...
    <div style="background:#f8fafc;border-radius:8px;padding:16px;border:1px solid #e2e8f0;margin-bottom:16px;">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:12px;">
        ${userArray.map((user, idx) => {
          ...
          const colors = ['#C1440E', '#0d9488', '#0891b2', '#0369a1', '#2563eb', '#7c3aed', '#d946ef', '#ec4899', '#f43f5e', '#f97316'];
    ...
    <div style="background:#f8fafc;border-radius:8px;padding:16px;border:1px solid #e2e8f0;">
      ...
      ${members.map(({ member: m, amount }, i) => `
        <div style="display:flex;align-items:center;gap:10px;padding:10px;background:#fff;border-radius:6px;margin-bottom:8px;">
          <div style="width:24px;height:24px;border-radius:50%;background:linear-gradient(135deg,#C1440E,#C1440E);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:11px;flex-shrink:0;">${i+1}</div>
          ${_avatarHtml(m, 32)}
          <div style="flex:1;min-width:0;">
