...
function _renderPagination(containerId, totalPages, currentPage, onPageFn) {
      ...
      document.getElementById("tb").innerHTML = items.length === 0
        ? `<tr><td colspan="9" style="text-align:center;padding:36px 20px;">
          <div class="rt-empty-msg">
            ...
            <button onclick="document.getElementById('user').focus()" style="background:#C1440E;color:#fff;border:none;padding:8px 18px;border-radius:8px;font-size:12px;cursor:pointer;font-family:inherit;">
    ...
    function openEditContrib(id) {
      ...
      <div class="_mft">
        ...
        <button class="_mbtn" style="background:#C1440E;" onclick="saveEditContrib('${id}')"><i class="fa-solid fa-check"></i> Save Changes</button>
      </div>`;
      ...
      document.getElementById("expenseRecordsBody").innerHTML = items.length === 0
        ? `<tr><td colspan="8" style="text-align:center;padding:36px 20px;">
          <div class="rt-empty-msg">
            ...
            <button onclick="document.getElementById('title').focus()" style="background:#C1440E;color:#fff;border:none;padding:8px 18px;border-radius:8px;font-size:12px;cursor:pointer;font-family:inherit;">
    ...
    function _exp_renderActiveTags(f) {
      ...
      box.innerHTML = tags.map(function(t, i){
        return '<span style="display:inline-flex;align-items:center;gap:4px;background:#fef9ec;border:1px solid #E0987A;border-radius:20px;padding:3px 10px;font-size:10px;font-weight:600;color:#92400e;cursor:pointer;" onclick="window._expClearTag(' + i + ')">' +
    ...
    function openEditExpense(id) {
      ...
      <div class="_mft">
        <button class="_mbtn" style="background:#999;" onclick="closeModal()">Cancel</button>
        <button class="_mbtn" style="background:#C1440E;" onclick="saveEditExpense('${id}','${e.Year || new Date().getFullYear()
    ...
    function _renderUsersPaged() {
      ...
      const statusBadge = (u) => {
        ...
        if (st === "pending") return `<span class="badge" style="background:#fff3e0;color:#e67e22;border:1px solid #C1440E;">Pending</span>`;
      ...
      <tr class="${rowClass}" onclick="viewUser('${u.UserId}')" title="Click to view details">
        ...
        <td onclick="event.stopPropagation();openEditUser('${u.UserId}')" title="Click to edit user" style="cursor:pointer;">
          <img src="${u.PhotoURL ? '' : _fbSvg}"
               ...
               width="32" height="32" style="border-radius:50%;object-fit:cover;background:#eee;border:2px solid #C1440E;display:block;"/>
        </td>
    ...
    function openEditUserWithPreview(
        ...
        <div style="text-align:center;margin-bottom:14px;">
          <div style="position:relative;width:72px;margin:0 auto 8px;">
            <img id="eu_photoPreview" src="${escapeHtml(photoSrc)}"
              ...
              style="width:72px;height:72px;border-radius:50%;object-fit:cover;border:3px solid #C1440E;background:#faeeda;display:block;margin-bottom:0;"/>
            <div onclick="document.getElementById('eu_photoFile').click()" style="position:absolute;bottom:1px;right:1px;width:22px;height:22px;background:#C1440E;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;border:2px solid white;box-shadow:0 1px 4px rgba(0,0,0,0.2);" title="Change Photo">
      ...
      <div class="_mft">
        ...
        <button class="_mbtn" style="background:#C1440E;" onclick="saveEditUser('${id}')"><i class="fa-solid fa-check"></i> Save Changes</button>
      </div>`;
      openModal(html, "460px");
    ...
    function _moveItem(listName, idx, dir) {
      ...
      if (savedEl) { savedEl.textContent = '⏳ Saving…'; savedEl.style.opacity = '1'; savedEl.style.color = '#C1440E'; }
    ...
    function openBulkInsert() {
      ...
      panelBody.innerHTML = `
        ...
        <div style="display:flex;align-items:flex-end;gap:10px;margin-bottom:14px;background:#fdf8ee;border-radius:10px;padding:12px 14px;border:1px solid #E0987A;">
          <div style="flex:1;">
          ...
          <button type="button" onclick="bkFillAllMonths()" style="background:#C1440E;box-shadow:none;padding:11px 16px;white-space:nowrap;flex-shrink:0;border-radius:9px;font-size:12.5px;color:#fff;border:none;cursor:pointer;font-family:Poppins,sans-serif;font-weight:700;">
            <i class="fa-solid fa-calendar-check"></i> Fill All 12
          </button>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
          <span style="font-size:12px;font-weight:700;color:#334155;"><i class="fa-solid fa-list-ul" style="color:#C1440E;margin-right:5px;"></i> Month Entries</span>
    ...
    async function runBulkInsert() {
      ...
      const previewHtml = `
          <div class="_mhdr"><h3><i class="fa-solid fa-eye" style="color:#C1440E;margin-right:6px;"></i> Preview & Confirm Bulk Insert</h3><button class="_mcls" onclick="closeModal()">&#xd7;</button></div>
          <div class="_mbdy">
            <div style="background:linear-gradient(135deg,#fef9ee,#fff8e1);border:1.5px solid #C1440E44;border-radius:12px;padding:12px 16px;margin-bottom:14px;font-size:12.5px;color:#946c44;">
      ...
      bkPanelBody.innerHTML = `
        ...
        <div style="background:linear-gradient(135deg,#fef9ee,#fff8e1);border:1.5px solid #C1440E55;border-radius:12px;padding:12px 16px;margin-bottom:16px;font-size:12.5px;color:#946c44;">
          <i class="fa-solid fa-circle-info"></i> Review details below. <b>Edit amounts inline</b> or remove a row before confirming.
        </div>
        ...
        <table style="width:100%;min-width:300px;border-collapse:collapse;" id="bkprev_table">
          ...
          <tbody>${previewRows}</tbody>
          <tfoot><tr style="background:#fef9ee;border-top:2px solid #E0987A;">
