(function() {
    ...
    function _showUndoToast(label, undoFn) {
      ...
      toastEl.innerHTML =
        ...
        '<div style="position:relative;flex-shrink:0;width:38px;height:38px;">' +
          '<svg width="38" height="38" viewBox="0 0 38 38" style="transform:rotate(-90deg);">' +
            '<circle cx="19" cy="19" r="' + R + '" fill="none" stroke="rgba(193, 68, 14,0.18)" stroke-width="3"/>' +
            '<circle id="_undoRing" cx="19" cy="19" r="' + R + '" fill="none" stroke="#C1440E" stroke-width="3"' +
            ' stroke-dasharray="' + CIRC + '" stroke-dashoffset="0" stroke-linecap="round"' +
            ' style="transition:stroke-dashoffset 0.1s linear;"/>' +
          '</svg>' +
          '<span style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#C1440E;" id="_undoSec">30</span>' +
        '</div>' +
        // Text
        '<div style="flex:1;min-width:0;line-height:1.4;">' +
          '<div style="font-size:11px;font-weight:600;color:#C1440E;letter-spacing:0.4px;text-transform:uppercase;margin-bottom:1px;">Deleted</div>' +
        ...
        '<button id="_undoBtn" onclick="_doUndo()" style="' +
          'background:linear-gradient(135deg,#C1440E,#f59e0b);border:none;color:#fff;' +
          'padding:7px 16px;border-radius:10px;cursor:pointer;font-weight:700;font-size:12px;' +
          'font-family:Poppins,sans-serif;box-shadow:0 4px 12px rgba(193, 68, 14,0.4);' +
      ...
      if (btn) {
        btn.onmouseenter = function(){ this.style.transform="scale(1.06)"; this.style.boxShadow="0 6px 18px rgba(193, 68, 14,0.55)"; };
        btn.onmouseleave = function(){ this.style.transform=""; this.style.boxShadow="0 4px 12px rgba(193, 68, 14,0.4)"; };
      }
      ...
      function _animateRing() {
        ...
        if (ring) ring.style.stroke = elapsed > DURATION * 0.83 ? "#ef4444" : "#C1440E";
        if (sec)  sec.style.color   = elapsed > DURATION * 0.83 ? "#ef4444" : "#C1440E";
        ...
        if (hasData) {
          ...
          if (contribPage && !document.getElementById("_draftBanner")) {
            ...
            banner.style.cssText =
              "background:linear-gradient(90deg,#fef9ec,#fef3c7);border:1px solid #E0987A;" +
